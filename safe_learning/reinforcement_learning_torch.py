...
# import utilities_torch as utilities
...
class PolicyIteration(object):
    ...
    def __init__(self, policy, dynamics, reward_function, value_function,
        ...
        self.state_space = torch.stack(state_space)
    ...
    def bellmann_error(self, states):
        ...
        # training target.
        with torch.no_grad():
            target = self.future_values(states)
        # Squared bellmann error
        return torch.sum(torch.square(target - self.value_function(states)))

    # @with_scope('value_iteration')
    def value_iteration(self):
        """Perform one step of value iteration."""
        future_values = self.future_values(self.state_space)
        return self.value_function.parameters[0].data.copy_(future_values)

    # @make_tf_fun(tf1.float64)
    ...
    def optimize_value_function(self, **solver_options):
        ...
        # return tf1.assign(self.value_function.parameters[0], values)  # 赋值语句
        return self.value_function.parameters[0].data.copy_(values)
    ...
    #         actions = tf1.placeholder(config.dtype,
    ...
    #         parameters = tf1.placeholder(config.dtype, action_array.shape)
    #         assign_op = tf1.assign(self.policy.parameters[0], parameters)
    #
    ...
    def discrete_policy_optimization(self, action_space, constraint=None):
        ...
        # Initialize
        values = torch.empty((n_states, n_options), dtype=config.np_dtype)
        action_array = torch.zeros(n_actions).expand(n_states, n_actions)
        ...
        for i, action in enumerate(action_space):
            ...
            action_array[:] = action
            # Compute values
            values[:, i] = self.future_values(states, actions=action_array)[:, 0]

            if constraint is not None:
                # TODO: optimize safety if unsafe
                unsafe = constraint(action_array) < 0
                values[unsafe, i] = float('-inf')

        # Select best action for policy
        best_actions = action_space[torch.argmax(values, axis=1)]
        self.policy.parameters[0].data.copy_(best_actions)